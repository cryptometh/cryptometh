...
// Copyright (c) 2013-2014 365Coin developers
...
uint256 hashGenesisBlock("0000001d5c6e0cdec35043518371327a42b83b4f0a5870b0eb92a3361cbc0f91");
static const unsigned int timeGenesisBlock = 1395954566;
static const unsigned int nNonceGenesisBlock = 98947241;
...
const string strMessageMagic = "CryptoMETH Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 1500 METH if made of 50 METH inputs)
            if (nBytes < 100)
                nMinFee = 0;
        }
        else
...
static const int64 nBlockRewardStartCoin = 326 * COIN;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("000000532b878669f19974cda0a7116309746e12ffd445f0744e951f8a6c6727");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Jesse, we need to cook!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        if (fTestNet)
        {
            block.nTime    = 1395954501;
            block.nNonce   = 46656023;
        }
        ...
        assert(block.hashMerkleRoot == uint256("5afcc3f019653613751221b0cc521aab8aaae5b561bd2fec7cec7b7f21d79edf"));
...
unsigned char pchMessageStart[4] = { 0x2b, 0xf2, 0xed, 0x4f };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoMETHMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoMETHMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CryptoMETHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptometh-miner");
    ...
    try { while(true) {
        ...
        printf("Running CryptoMETHMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("CryptoMETHMiner terminated\n");
        throw;
    }
}
